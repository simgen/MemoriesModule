...
public class Api {
    ...
    public interface SearchCallback {
        public void onSearchCompleted(ArrayList<Photo> photos);
    ...
    private static String getSearchUrl(String tag) {

        return getUrlForMethod("flickr.photos.search") + "&tags=" + tag + "&min_upload_date=" + getNDaysBeforeDate(30) + "&sort=interestingness-desc&per_page=500";
    }

    private static String getNDaysBeforeDate(int numOfDaysBefore)
    {
    ...
    private static String getInterestingPhotosUrl(){
        return getUrlForMethod("flickr.interestingness.getList");
    }
    ...
    public void search(String text, final SearchCallback cb) {
        StringRequest request = new StringRequest(Request.Method.GET, getSearchUrl(text), new Response.Listener<String>() { //getSearchUrl(text), new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                try {
                    ...
                    ArrayList<Photo> results = new ArrayList<Photo>(photos.length());
